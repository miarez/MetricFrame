...
export class Table {
  ...
  build() {
    ...
    const core = new Core(this._rows, {
    ...
    if (this._heatmapMode === "global") {
      core.applyHeatmapGlobal();
    } else if (this._heatmapMode === "perColumn") {
      core.applyHeatmapPerColumn();
    } else if (this._heatmapMode === "byDimension" && this._heatmapDimKey) {
      core.applyHeatmapByDimension(this._heatmapDimKey, {
    ...
    for (const { dimKey, options } of this._groupBorders) {
      core.applyGroupBorders(dimKey, options);
    }
